...
bool hSolve(long long board, vector<Move>& solution, HashType<long long>& H, int j, vector<vector<int>>& search, vector<vector<int>>& insert);
...
int main()
{
    ...
    double time;
    long long board = 0;
    vecror<vector <int>> SearchProbes;
    vecror<vector <int>> InsertProbes;
    ...
    while (looper && cin >> choice)
    {
        ...
        switch (choice)
        {
            ...
            case 1:
            {
                ...
                if (hSolve(board, moves, hashTable, 1, SearchProbes, InsertProbes))
                {
            ...
            case 2:
            {
                ...
                LinearHashTable<long long> LhashTable;
                start_time = clock();
                
                // if board was solved by hSolve
                if (hSolve(board, moves, LhashTable, 2, SearchProbes, InsertProbes))
                {
            ...
            case 3:
            {
                ...
                QuadraticProbingTable<long long> QhashTable;
                start_time = clock();
                
                // if board was solved by hSolve
                if (hSolve(board, moves, QhashTable, 3, SearchProbes, InsertProbes))
                {
            ...
            case 4:
            {
                ...
                // if board was solved by hSolve
                if (hSolve(board, moves, DhashTable, 4, SearchProbes, InsertProbes))
                {
        ...
        if (looper)
        {
            ...
            // Clears the Search & Insert Probes for another round of
            // the program running.
            for (int i = 0; i < SearchProbes.size(); i++)
            {
                for (int k = 0; k < SearchProbes[i].size(); k++)
                {
                    SearchProbes[i][k] = 0;
                }
            }
            for (int i = 0; i < InsertProbes.size(); i++)
            {
                for (int k = 0; k < InsertProbes[i].size(); k++)
                {
                    InsertProbes[i][k] = 0;
                }
            }
...
bool hSolve(long long board, vector<Move>& solution, HashTable<long long>& H, int j, vector<vector<int>>& search, vector<vector<int>>& insert)
{
    ...
    for (auto m: currMoves)
    {
        ...
        // user wants a separate chaining solve
        if (j == 1)
        {
            
            // if H doesn't contain the updated board
            if (!H.containsSeparate(m.board, NumProbes))
            {
                
                search[SearchRow++][1] = NumProbes;
                
                // if the board was solved in the next call of hsolve then
                // record the solution to solution vector and return true
                if (hSolve(m.board, solution, H, j, search, insert, probes))
                {
                ...
                else
                {
                    ...
                    H.insert(m.board, NumProbes);
                    insert[InsertRow++][1] = NumProbes;
                    
                }
                
            } // end of if
            
        } // end of separate chaining solver
        
        // user wants a linear Hashing solve
        else if (j == 2)
        {
            
            // if H doesn't contain the updated board
            if (!H.containsLinear(m.board, NumProbes))
            {
                
                search[SearchRow++][2] = NumProbes;
                
                // if the board was solved in the next call of hsolve then
                // record the solution to solution vector and return true
                if (hSolve(m.board, solution, H, j, search, insert))
                {
                ...
                else
                {
                    ...
                    H.insert(m.board, NumProbes);
                    insert[InsertRow++][2] = NumProbes;

                }
            
        } // end of linear hashing solver
        
        // user wants a Quadratic Probing solve
        else if (j == 3)
        {
            
            // if H doesn't contain the updated board
            if (!H.containsQuadratic(m.board, NumProbes))
            {
                
                search[SearchRow++][3] = NumProbes;
                ...
                else
                {
                    ...
                    H.insert(m.board, NumProbes);
                    insert[InsertRow++][3] = NumProbes;

                    
                }
            
        } // end of Quadratic Probing solver
        
        // user wants a Double Hashing solve
        else if (j == 4)
        {
            
            // if H doesn't contain the updated board
            if (!H.containsDouble(m.board, NumProbes))
            {
                
                search[SearchRow++][4] = NumProbes;
                ...
                else
                {
                    ...
                    H.insert(m.board, NumProbes);
                    insert[InsertRow++][4] = NumProbes;

                    
                }
            
        } // end of Double hashing solver
        
        // User inserted an incorrect option and bypassed original
        // error handler...take no prisoners...
        else
        {
            cout << "\nUser inserted incorrect option...\n"
            return false;
